...
assert.match(script, /createTerrainHorizonLodKit/, "V2 should compose terrain horizon LOD directly.");
assert.match(script, /createWorldPatchKit/, "V2 should compose generic world patch streaming directly.");
assert.match(script, /createDomainTerrainSampler/, "V2 should additively wrap terrain sampling through terrain domain kits.");
assert.match(script, /createHorizonRingGeometry/, "V2 should render compressed horizon terrain rings.");
...
assert.match(script, /terrainShapingReady:\s*Boolean/, "V2 validation should expose terrain shaping readiness.");
assert.match(script, /terrainHydrologyReady:\s*Boolean/, "V2 validation should expose terrain hydrology readiness.");
assert.match(script, /terrainHorizonReady:\s*Boolean/, "V2 validation should expose terrain horizon readiness.");
assert.match(script, /horizonRingsPresent:/, "V2 validation should expose horizon ring readiness.");
