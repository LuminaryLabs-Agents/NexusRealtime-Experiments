...
import { MEADOW_SEED, terrainHeight } from "./meadow-experiment-scene.js?v=0.0.2-cutover-1";

export const SUN = new THREE.Vector3(0.38, 0.88, 0.22).normalize();
const litVertex = `varying vec3 vNormal; varying vec3 vWorld; void main(){vNormal=normalize(normalMatrix*normal);vec4 w=modelMatrix*vec4(position,1.);vWorld=w.xyz;gl_Position=projectionMatrix*viewMatrix*w;}`;
...
export function createSky(uniforms) {
  return new THREE.Mesh(new THREE.SphereGeometry(260, 48, 24), new THREE.ShaderMaterial({
    side: THREE.BackSide,
    uniforms: uniforms({ uZenith: { value: new THREE.Color(0x6aa1e0) }, uHorizon: { value: new THREE.Color(0xffbb75) } }),
...
export function installControls(canvas, camera, target, options = {}) {
  ...
  return { control, update, seed: MEADOW_SEED };
}
