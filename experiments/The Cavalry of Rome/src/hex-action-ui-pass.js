...
function startAction(actionId){const snapshot=tacticalSnapshot();const card=cards.get(actionId);if(card?.disabled||card?.dataset.spent==="true")return;if(actionId==="rollAp"&&(rollApSpent||!snapshot?.canRollInPlace))return;const result=actionId==="rollAp"?globalThis.GameHost?.rollActionPointsInPlace?.():globalThis.GameHost?.startManeuver?.(actionId);if(actionId==="rollAp"&&result!==false&&result!=null)rollApSpent=true;if(result===false||result==null){card?.animate?.([{transform:"translateX(0)"},{transform:"translateX(-5px)"},{transform:"translateX(5px)"},{transform:"translateX(0)"}],{duration:180,easing:"ease-out"})}}
...
function render(){ensureUi();const gameSnapshot=globalThis.GameHost?.getSnapshot?.()??{};const snapshot=tacticalSnapshot();const visible=gameSnapshot.mode==="battlefield";if(visible!==lastVisible){root.dataset.visible=visible?"true":"false";lastVisible=visible}if(visible){if(snapshot?.turn!==lastTurn){lastTurn=snapshot?.turn;rollApSpent=!snapshot?.canRollInPlace}if(snapshot?.side==="enemy")rollApSpent=true;const ap=Number(snapshot?.actionPoints??0);apValue.textContent=String(ap);phaseValue.textContent=labelForPhase(snapshot);const locked=snapshot?.side==="enemy"||Boolean(snapshot?.gameOver);passButton.disabled=locked||Boolean(snapshot?.activeManeuver);concedeButton.disabled=Boolean(snapshot?.gameOver);for(const action of ACTIONS){const card=cards.get(action.id);const active=snapshot?.activeManeuver===action.id;card.dataset.active=active?"true":"false";const rollLocked=action.id==="rollAp"&&(rollApSpent||Boolean(snapshot?.activeManeuver)||!snapshot?.canRollInPlace);card.dataset.spent=rollLocked?"true":"false";card.disabled=locked||(action.id==="rollAp"?rollLocked:Boolean(snapshot?.activeManeuver&& !active)||ap<action.cost);card.setAttribute("aria-pressed",active?"true":"false")}}requestAnimationFrame(render)}
